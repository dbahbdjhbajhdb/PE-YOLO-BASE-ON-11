...
class CE_Head(Detect):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        # 2) 按你自己的设计重写 reg_max / dfl
        self.reg_max = 16
        self.no = self.nc + self.reg_max * 4
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()

        self.nl = len(ch)  # 检测层个数
        c2 = max(16, ch[0] // 4, self.reg_max * 4)
        c3 = max(ch[0], min(self.nc, 100))

        # 3) 你的额外分支（dilated depthwise 之类）
...
class R3Head(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            # 回归 residual expert
            r0 = self.cv2[i][0](xi)
            r_res = self.cv2_[i](r0) + r0
            r1 = self.cv2[i][1](r_res)
            r = self.cv2[i][2](r1)
            r_refined = r + self.res_refine[i](r)
            ...
            x[i] = torch.cat([r_refined, c], 1)

        # 跨尺度 residual 融合 (尺寸自适应)
        for i in range(self.nl):
            tmp = 0
...
class StripContext(nn.Module):
    ...
    def __init__(self, c):
        ...
        self.dw_v = nn.Conv2d(c, c, kernel_size=(5, 1), padding=(2, 0), groups=c, bias=False)
...
class ScaleRouteFusion(nn.Module):
    ...
    def __init__(self, c):
        super().__init__()
        hidden = max(c // 4, 16)
        self.fc1 = nn.Conv2d(3 * c, hidden, 1, bias=False)
        self.act = nn.SiLU()
        self.fc2 = nn.Conv2d(hidden, 3 * c, 1, bias=True)
        self.mix = Conv(c, c, 3)

    def forward(self, cur, prev=None, nxt=None):
        b, c, h, w = cur.shape

        if prev is None:
            prev = torch.zeros_like(cur)
        else:
            prev = F.interpolate(prev, size=(h, w), mode="nearest")
        ...
        out = wgt[:, 0] * cur + wgt[:, 1] * prev + wgt[:, 2] * nxt
        return self.mix(out)
...
class TaskAlignInteract(nn.Module):
    ...
    def __init__(self, c_shared, c_reg, c_cls):
        ...
        self.shared = Conv(c_shared, c_shared, 3)

        self.reg_stem = Conv(c_shared, c_reg, 3)
        self.cls_stem = Conv(c_shared, c_cls, 3)

        self.reg2cls = nn.Conv2d(c_reg, c_cls, 1, bias=False)
        self.cls2reg = nn.Conv2d(c_cls, c_reg, 1, bias=False)

        self.align_map = nn.Conv2d(c_shared, 2, 1, bias=True)
        self.spatial_prior = nn.Conv2d(c_shared, 1, 3, padding=1, bias=True)

        self.cls_context = StripContext(c_cls)
        self.cls_refine = Conv(c_cls, c_cls, 3)

        self.reg_refine = Conv(c_reg, c_reg, 3)

    def forward(self, x):
        shared = self.shared(x)

        reg = self.reg_stem(shared)
        cls = self.cls_stem(shared)

        align = torch.sigmoid(self.align_map(shared))
        a_reg = align[:, 0:1]
        a_cls = align[:, 1:2]

        reg = reg + self.cls2reg(cls) * a_reg
        cls = cls + self.reg2cls(reg) * a_cls

        prior = torch.sigmoid(self.spatial_prior(shared))
        reg = reg * (1.0 + prior)
        reg = self.reg_refine(reg)

        cls = cls + self.cls_context(cls)
        cls = self.cls_refine(cls)
...
class RDC_Head(Detect):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        c_shared = max(64, min(ch))
        c_reg = max(64, self.reg_max * 4)
        c_cls = max(64, min(128, c_shared))

        # 各层先映射到统一维度，便于跨层路由
        self.stem = nn.ModuleList(Conv(x, c_shared, 1) for x in ch)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            prev_feat = feats[i - 1] if i > 0 else None
            next_feat = feats[i + 1] if i < self.nl - 1 else None

            fused = self.route[i](feats[i], prev_feat, next_feat)
            reg_feat, cls_feat = self.task[i](fused)

            reg_out = self.bbox_pred[i](reg_feat)
            cls_out = self.cls_pred[i](cls_feat)
...
# --- 1. CFDH 核心模块 (增强健壮性) ---
# --- 把这个类完全替换进去 ---
class CFDH_SingleHead(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        # [Debug] 替换为普通卷积，排查是否是 DCN 算子导致崩溃
        # self.dcn1_offset = nn.Conv2d(c1, 18, 3, padding=1)
        # self.dcn1 = DeformConv2d(c1, c1, 3, padding=1)
        self.dcn1 = nn.Conv2d(c1, c1, 3, padding=1)  # 临时替换

        self.stem = Conv(c1, c2, 1)

        # Extraction
        self.cls_convs = nn.Sequential(DWConv(c2, c2, 3), DWConv(c2, c2, 3))
        self.reg_convs = nn.Sequential(DWConv(c2, c2, 3), DWConv(c2, c2, 3))

        # Adjustment
        self.reg_init_conv = nn.Conv2d(c2, 4, 1)
        self.geo_offset_generator = nn.Conv2d(4, 18, kernel_size=3, padding=1)

        # [Debug] 替换为普通卷积
        # self.dcn_cls_refine = DeformConv2d(c2, c2, 3, padding=1)
        # self.dcn_reg_refine = DeformConv2d(c2, c2, 3, padding=1)
        self.dcn_cls_refine = nn.Conv2d(c2, c2, 3, padding=1)  # 临时替换
        self.dcn_reg_refine = nn.Conv2d(c2, c2, 3, padding=1)  # 临时替换

    def forward(self, x):
        # [Debug] 不需要计算 offset，直接过普通卷积
        # offset1 = self.dcn1_offset(x)
        # x = self.dcn1(x, offset1)
        x = self.dcn1(x)
        ...
        # reg_init = self.reg_init_conv(x_reg)
        # geo_offset = self.geo_offset_generator(reg_init)

        # [Debug] 直接过普通卷积
        # x_cls_geo = self.dcn_cls_refine(x_cls, geo_offset)
        # x_reg_geo = self.dcn_reg_refine(x_reg, geo_offset)
        x_cls_geo = self.dcn_cls_refine(x_cls)
        x_reg_geo = self.dcn_reg_refine(x_reg)
...
class Detect_CFDH(Detect):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            # 特征提取
            x_reg_geo, x_cls_geo = self.cfdh_modules[i](x[i])

            # 投影
            box = self.cv2[i](x_reg_geo)
            cls = self.cv3[i](x_cls_geo)
    ...
    def bias_init(self):
        ...
        for i in range(m.nl):
            # 获取对应的 Conv2d 层
            # 因为我们用 Sequential 包了一层，所以取 [0] 或 [-1] 都可以
            # 这里用 getattr/hasattr 动态检查，确保万无一失

            # --- 初始化 Box 分支 ---
            cv2_module = m.cv2[i]
            if isinstance(cv2_module, nn.Sequential):
                conv_box = cv2_module[-1]
            else:
                conv_box = cv2_module
            ...
            # --- 初始化 Cls 分支 ---
            cv3_module = m.cv3[i]
            if isinstance(cv3_module, nn.Sequential):
                conv_cls = cv3_module[-1]
            else:
                conv_cls = cv3_module
...
class DeepSupervisionDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), aux_weight=0.25):
        super().__init__()
        assert len(ch) == 6, f"DeepSupervisionDetect expects 6 input feature maps, got {len(ch)}"
        self.aux_weight = aux_weight
        self.aux_detect = Detect(nc=nc, ch=ch[:3])
        self.main_detect = Detect(nc=nc, ch=ch[3:])

    def forward(self, x):
        assert isinstance(x, (list, tuple)) and len(x) == 6
        aux_x = x[:3]
        main_x = x[3:]

        main_pred = self.main_detect(main_x)

        if self.training:
            aux_pred = self.aux_detect(aux_x)
            return {
                "main": main_pred,
                "aux": aux_pred,
