...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
        ...
        elif m is Conv1x1_Equal:
            c2 = ch[f]
            args = [c2]
        elif m is DynamicAttention:
            c1 = ch[f]
            c2 = c1  # 自动让输出通道 = 输入通道
            args = [c1, c2, *args]
            # --- 修正后的 Detect_CFDH 分支 ---
            ...
            c2 = args[0]
            args = [c1, c2, *args[1:]]
        elif m is Detect_CFDH:
            args.append([ch[x] for x in f])
        elif m is RHDWT:
            c1 = ch[f]
            c2 = args[0]

            # 确保 gw (width_multiple) 已定义。
            # 在 parse_model 函数开头通常有: gd, gw, act = d['depth_multiple'], d['width_multiple']...
            # 如果你的版本里叫 width，就把下面的 gw 改成 width
            ...
            c2 = c1  # 自动让输出通道 = 输入通道
            args = [c1, c2, *args]
        elif m is SDC_Selective_Fusion:
            c1 = [ch[x] for x in f]
            c2 = c1[1] // 2
            args = [c1, c2]
        elif m is VGASBlock:
            ...
            out_ch = args[0] if len(args) > 0 else 64
            stack_times = args[1] if len(args) > 1 else 1
            num_outs = args[2] if len(args) > 2 else 5
            drop_prob = args[3] if len(args) > 3 else 0.05
            block_size = args[4] if len(args) > 4 else 3

            # 不要给 NASFPN 再乘 width
            args = [in_ch, out_ch, num_outs, stack_times, drop_prob, block_size]
            c2 = [out_ch] * num_outs
        elif m is DeepSupervisionDetect:
            nc = args[0]
            aux_weight = args[1] if len(args) > 1 else 0.25
            args = [nc, [ch[x] for x in f], aux_weight]
            c2 = None
        elif m is PolarizedAttention:
            c1 = ch[f]
            c2 = c1  # 自动让输出通道 = 输入通道
            args = [c1, c2, *args]
        elif m is EdgeSkipGate:
            c1 = ch[f]
