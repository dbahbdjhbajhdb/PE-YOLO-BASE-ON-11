...
# from ultralytics.nn.tasks import attempt_load_weights # 已删除无效导入
# from ultralytics.utils.torch_utils import intersect_dicts # 已删除无效导入
...
class yolo_heatmap:
    ...
    def post_process(self, result):
        # 这里的 non_max_suppression 其实也没用到，因为 predict 已经做了
    ...
    def process(self, img_path, save_path):
        ...
        # ==============================================================================
        # 关键修改点：当 show_result 为 False 时，跳过绘制检测框
        # ==============================================================================
        if self.show_result:
            # 只有在需要绘制框的时候才进行预测和绘制
...
def get_params():
    params = {
        "weight": r"C:\ultralytics-main\runs\detect\YOLO11(L)-VISDRONE2019-300\weights\best.pt",
        "device": "cuda:0",
        "method": "GradCAMPlusPlus",  # 对于小目标，GradCAMPlusPlus 通常比 EigenCAM 效果好一点
        # 'layer': [10, 12, 14, 16, 18], # 原来的层（偏深）
        # 建议修改：加入一些中浅层，更容易捕捉小目标信息
        "layer": [6, 8, 10, 12, 15, 18, 21],
        "backward_type": "all",
        "conf_threshold": 0.2,
        "ratio": 0.1,  # 稍微调大一点，让更多目标参与计算
        # =======================================================
        # 核心修改：设置为 False，不绘制检测框
        "show_result": False,
        # =======================================================
        "renormalize": False,
        "task": "detect",
...
if __name__ == "__main__":
    # 请确保图片路径正确
    img_path = r"C:\date\Visdrone2019\images\test2019\0000074_06746_d_0000013.jpg"
    # 结果保存目录
    save_dir = "heatmap_result_clean"
